...
loadData           <- function(objj, 
    ...
    # get PCs    
    h.pcs <- Embeddings(obj[["harmony"]])
    
    # get expression
    activity <- obj@assays$RNA$data
...
validateCluster    <- function(df){
    if(! "Cluster" %in% colnames(df)){
        if("leiden_clusters" %in% colnames(df)){
            df$Cluster <- paste0("cluster", as.numeric(as.factor(df$leiden_clusters)))
            return(df)
          ...
          return(df)
        }else if("seurat_clusters" %in% colnames(df)){
          df$Cluster <- paste0("cluster",df$seurat_clusters)
          return(df)
        }
    }else{
...
normalize.activity <- function(df, 
    ...
    saveRDS(norm.act, file=paste0(output,".normalizedActivity.rds"))
...
plot.act.scores    <- function(df, 
    ...
    # params
    png(file=outname, width=8, height=ratio*8, units="in", res=400, type="cairo")
    ...
    for (i in 1:length(gids)){
        ...
        #cols <- inferno(100)
        #cols <- plasma(100)
        cols <- colorRampPalette(c("grey80", brewer.pal(9, "YlGnBu")[3:9]), bias=0.75)(100)
	...
	colvec[is.na(colvec) & acv == 0] <- cols[1]
        ...
        plot(df2$umap1, df2$umap2, col=colvec,
             main=paste0(info$name[i],"-",info$type[i]),
             cex.main=0.5,
             xlab="", ylab="", bty="n",
...
runMajorPriori     <- function(all.b, 
    ...
    saveRDS(impute.activity, file=paste0(output,".smoothed.markers.rds"))
